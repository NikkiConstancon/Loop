...
char MAC_SCALE[14] = "881B990960B4";
        ...
        if (MySignals_BLE.attributeWrite(MySignals_BLE.connection_handle, SCALE_PROFILE_HANDLE, scale_user_profile, 8) == 0)
        {
          ...
          do
          {
            if (MySignals_BLE.waitEvent(1000) == BLE_EVENT_ATTCLIENT_ATTRIBUTE_VALUE)
            {
              ...
              Serial.println();
              Serial.print("SCALW");
              Serial.println((float(scaleData.weight) / 10), 1);
              delay(10);

              //Serial.println(F("Body Fat: "));
              Serial.print("SCABF");
              Serial.println((float(scaleData.bodyfat) / 10), 1);
              delay(10);

              //Serial.println(F("Bone Mass: "));
              Serial.print("SCALB");
              Serial.println((float(scaleData.bonemass) / 10), 1);
              delay(10);

              //Serial.println(F("Muscle mass: "));
              Serial.print("SCAMM");
              Serial.println((float(scaleData.musclemass) / 10), 1);
              delay(10);

              //Serial.print(F("Visceral fat: "));
              Serial.print("SCAVF");
              Serial.println(scaleData.visceralfat);
              delay(10);

              //Serial.print(F("Water percentage: "));
              Serial.print("SCABW");
              Serial.println((float(scaleData.water) / 10), 1);
              delay(10);

              //Serial.print(F("Calories: "));
              Serial.print("SCALC");
              Serial.println(scaleData.calories);
